...
#from datetime import dat (TODO: some of the modules might be mentioned twice, so fix this)
...
while True:
    ...
    chosen_calculation = pyip.inputMenu(['A', 'B'], lettered= False, numbered = False)

    if chosen_calculation == 'A':
        #Let user input 2 dates (the following format works too for the dates: "11/03/21" or "11/3/21)
        chosen_date_1 = pyip.inputDate('Type in the first date in the format "MM/DD/YYYY" (Month-Day-Year) for example "11/03/2021" for November, 3rd 2021) (> ')

        chosen_date_2 = pyip.inputDate('Type in the second date in the format "MM/DD/YYYY" (Month-Day-Year) for example "11/03/2021" for November, 3rd 2022) (> ')

        #Calculate the amout of time between the 2 dates (with datetime module)
        ...
        #Output
        print("---The amount of time between the 2 dates (", chosen_date_earlier, "and", chosen_date_later, ") is:", round(date_difference/delta_1_year, 2), "years or", round(date_difference/delta_1_week, 2), "weeks or", round(date_difference/delta_1_day, 2), "days or", round(date_difference/delta_1_hour, 2), "hours" ,round(date_difference/delta_1_minute, 2), "minutes or", round(date_difference/delta_1_second, 2), "seconds---")  
        
    if chosen_calculation == 'B':
        #User chooses the first date (the following format works too for the dates: "11/03/21" or "11/3/21)
        chosen_date_1 = pyip.inputDate('Type in the date in the format "MM/DD/YYYY" (Month-Day-Year) for example "11/03/2021" for November, 3rd 2021) (> ')
        ...
        choice_future_past_input = pyip.inputMenu(['future', 'past'], lettered=True, numbered=False) 

        #User types in how many days the date is supposed to be in the past/future and the new date is displayed.
        print("Please type the number of days you want to calculate from", chosen_date_1, "into the", choice_future_past_input,".")
        ...
        print("---The new date which is", days_difference, "days in the", choice_future_past_input, "of", chosen_date_1, "is:", new_date, "---")
